...
test('scraper can get first tweet matching query', async () => {
    ...
    const latestQuote = await scraper.getTweetWhere(timeline, {
        isQuoted: true,
    });
...
test('scraper can get tweet quotes without logging in', async () => {
    const expected: Tweet = {
        ...
        name: 'Vsauce2',
        permanentUrl:
            'https://twitter.com/VsauceTwo/status/1237110546383724547',
        photos: [
            {
...
test('scraper can get retweet', async () => {
    const expected: Tweet = {
        ...
        name: 'federico.',
        permanentUrl:
            'https://twitter.com/federicosmos/status/1776276954435481937',
        photos: [],
...
test('scraper can get a tweet with getTweetV2', async () => {
    ...
    if (shouldSkipV2Tests) {
        return console.warn(
            "Skipping 'getTweetV2' test due to missing API keys.",
        );
    }
...
test('scraper can send a tweet with image and video', async () => {
    const scraper = await getScraper();
    const draftText =
        'Test tweet with image and video ' + Date.now().toString();
