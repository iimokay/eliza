...
export class HlsRecordPlugin implements Plugin {
    ...
    private async handleOccupancyUpdate(update: OccupancyUpdate) {
        ...
        try {
            const status = await scraper.getAudioSpaceStreamStatus(
                this.mediaKey,
            );
            ...
            this.logger?.error(
                '[HlsRecordPlugin] handleOccupancyUpdate =>',
                err,
            );
        }
    }
    ...
    private async startRecording(hlsUrl: string): Promise<void> {
        ...
        this.logger?.info(
            '[HlsRecordPlugin] Starting HLS recording =>',
            hlsUrl,
        );
        ...
        this.recordingProcess.on('error', (err) => {
            this.logger?.error(
                '[HlsRecordPlugin] Recording process failed =>',
                err,
            );
        });
    }
