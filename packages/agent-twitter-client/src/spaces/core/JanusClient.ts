...
export class JanusClient extends EventEmitter {
    ...
    public async subscribeSpeaker(userId: string): Promise<void> {
        ...
        this.logger.debug(
            '[JanusClient] subscriber => done (user=',
            userId,
            ')',
        );
    ...
    pushLocalAudio(samples: Int16Array, sampleRate: number, channels = 1) {
        if (!this.localAudioSource) {
            this.logger.warn(
                '[JanusClient] No localAudioSource; enabling now...',
            );
            this.enableLocalAudio();
        }
    ...
    private async createRoom() {
        ...
        if (!resp.ok) {
            throw new Error(
                `[JanusClient] createRoom failed => ${resp.status}`,
            );
        }
        const json = await resp.json();
    ...
    private async joinRoom(): Promise<number> {
        ...
        this.logger.debug(
            '[JanusClient] joined room => publisherId=',
            publisherId,
        );
        return publisherId;
    }
    ...
    public async destroyRoom(): Promise<void> {
        ...
        if (!resp.ok) {
            throw new Error(
                `[JanusClient] destroyRoom failed => ${resp.status}`,
            );
        }
        const json = await resp.json();
    ...
    public async leaveRoom(): Promise<void> {
        ...
        if (!resp.ok) {
            throw new Error(
                `[JanusClient] leaveRoom => error code ${resp.status}`,
            );
        }
        const json = await resp.json();
