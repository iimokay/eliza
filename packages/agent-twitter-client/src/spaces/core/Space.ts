...
export class Space extends EventEmitter {
    ...
    async initialize(config: SpaceConfig) {
        ...
        this.janusClient.on(
            'audioDataFromSpeaker',
            (data: AudioDataWithUser) => {
                this.logger.debug(
                    '[Space] Received PCM from speaker =>',
                    data.userId,
                );
    ...
    private async callApproveEndpoint(
        ...
        const endpoint =
            'https://guest.pscp.tv/api/v1/audiospace/request/approve';
        const headers = {
    ...
    private async callRemoveEndpoint(
        ...
        this.logger.debug(
            '[Space] Speaker removed => sessionUUID=',
            sessionUUID,
        );
    }
    ...
    public async finalizeSpace(): Promise<void> {
        this.logger.info(
            '[Space] finalizeSpace => stopping broadcast gracefully',
        );
    ...
    private async endAudiospace(params: {
        ...
        const url =
            'https://guest.pscp.tv/api/v1/audiospace/admin/endAudiospace';
        const headers = {
            'Content-Type': 'application/json',
            Referer: 'https://x.com/',
        ...
        if (!resp.ok) {
            ...
            throw new Error(
                `[Space] endAudiospace => ${resp.status} ${errText}`,
            );
        }
